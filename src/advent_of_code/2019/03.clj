...
(defn next-coords [coord code]
  (let [[dir n] [(subs code 0 1) (Long/parseLong (subs code 1))]
        v (case dir
            "U" [0 1]
            "D" [0 -1]
            "L" [-1 0]
            "R" [1 0])]
    (reduce (fn [acc _] (conj acc (v-add v (peek acc)))) [coord] (range n))))

(defn line-coords [line]
  (:coords
   (reduce (fn [{:keys [coords coord]} code]
             (let [coords' (next-coords coord code)]
               {:coords (concat coords (rest coords'))
                :coord (peek coords')}))
           {:coords '() :coord [0 0]}
           (str/split line #","))))
...
(defn coord-steps [coords]
  (into {} (reduce (fn [acc [i c]] (merge-with min acc {c (inc i)})) {} (map-indexed vector coords))))

(defn part-one [line-1 line-2]
  (let [coords-1 (line-coords line-1)
        coords-2 (line-coords line-2)
        intersection-points (set/intersection (set coords-1)
                                              (set coords-2))]
    (->> (set/difference intersection-points #{[0 0]})
         (sort-by (fn [coord] (manhattan [0 0] coord)))
         (first)
         (manhattan [0 0]))))

(defn part-two [line-1 line-2]
  (let [coords-1 (line-coords line-1)
        coords-2 (line-coords line-2)
        steps-1 (coord-steps coords-1)
        steps-2 (coord-steps coords-2)
        intersection-points (set/intersection (set coords-1)
                                              (set coords-2))]
    (->> (set/difference intersection-points #{[0 0]})
         (sort-by (fn [coord] (+ (get steps-1 coord) (get steps-2 coord))))
         (map (juxt identity (fn [coord] (+ (get steps-1 coord) (get steps-2 coord)))))
         (first))))

(comment
  ;; Part One
  (part-one line-1 line-2) ;;; => 1225

  (second (part-two line-1 line-2)) ;; => 107036
  )
